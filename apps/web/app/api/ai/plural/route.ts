...
import { generatePluralExercises } from "@/lib/ai-content";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exercises = await generatePluralExercises(grade, count, userId);
    return NextResponse.json(exercises);
  } catch (err) {
    console.error("Plural route error:", err);
    void logError({ errorType: "plural_route_error", errorMessage: String(err).slice(0, 500), page: "uebungen" });
