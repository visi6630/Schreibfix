...
import { generateSatzzeichenExercises } from "@/lib/ai-content";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exercises = await generateSatzzeichenExercises(grade, count, userId);
    return NextResponse.json(exercises);
  } catch (err) {
    console.error("Satzzeichen route error:", err);
    void logError({ errorType: "satzzeichen_route_error", errorMessage: String(err).slice(0, 500), page: "uebungen" });
