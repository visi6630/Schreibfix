...
import { generateGrossschreibungExercises } from "@/lib/ai-content";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exercises = await generateGrossschreibungExercises(grade, count, userId);
    return NextResponse.json(exercises);
  } catch (err) {
    console.error("Grossschreibung route error:", err);
    void logError({ errorType: "grossschreibung_route_error", errorMessage: String(err).slice(0, 500), page: "uebungen" });
