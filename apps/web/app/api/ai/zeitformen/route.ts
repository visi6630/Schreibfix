...
import { generateZeitformenExercises } from "@/lib/ai-content";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exercises = await generateZeitformenExercises(grade, count, userId);
    return NextResponse.json(exercises);
  } catch (err) {
    console.error("Zeitformen route error:", err);
    void logError({ errorType: "zeitformen_route_error", errorMessage: String(err).slice(0, 500), page: "uebungen" });
