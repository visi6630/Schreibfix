...
import { generateWortartenExercises } from "@/lib/ai-content";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exercises = await generateWortartenExercises(grade, count, userId);
    return NextResponse.json(exercises);
  } catch (err) {
    console.error("Wortarten route error:", err);
    void logError({ errorType: "wortarten_route_error", errorMessage: String(err).slice(0, 500), page: "uebungen" });
