...
import { generateSteigerungExercises } from "@/lib/ai-content";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const exercises = await generateSteigerungExercises(grade, count, userId);
    return NextResponse.json(exercises);
  } catch (err) {
    console.error("Steigerung route error:", err);
    void logError({ errorType: "steigerung_route_error", errorMessage: String(err).slice(0, 500), page: "uebungen" });
